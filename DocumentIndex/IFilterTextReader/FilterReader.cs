    ...
    public class FilterReader : TextReader
    {
                            ...
                            FilterReaderOptions filterReaderOptions = null)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrWhiteSpace(extension))
                        throw new IFFilterNotFound("没有安装用来解析'" + Path.GetFileName(fileName) + "'文件的" + (Environment.Is64BitProcess ? "64" : "32") +
                                                   "位IFilter程序 ");

                    throw new IFFilterNotFound("没有安装用来解析拓展名为'" + extension + "'文件的" + (Environment.Is64BitProcess ? "64" : "32") +
                                               " 位IFilter程序");
                }
            
                if (_options.ReaderTimeout != FilterReaderTimeout.NoTimeout && _options.Timeout < 0)
                    throw new ArgumentException("Needs to be larger then 0", "_options.Timeout");
            }
            catch (Exception)
            {
                Dispose();
        ...
        {
            if (string.IsNullOrWhiteSpace(extension))
                throw new ArgumentException("扩展名不能为空", "扩展名");
